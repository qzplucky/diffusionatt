...
def DWConv(in_channels, out_channels, kernel, padding):
...
class ResBlock(TimestepBlock):
    ...
    def __init__(
        ...
        #     normalization(channels),
        #     nn.SiLU(),
        #     DWConv(channels, self.out_channels, kernel=3, padding=1),
        # )
        ...
        if emb_channels > 0:
            ...
            #     nn.Dropout(p=dropout),
            #     zero_module(
            #         DWConv(self.out_channels, self.out_channels, kernel=3, padding=1)
            #     ),
            # )
        ...
        if self.out_channels == channels:
            ...
            self.skip_connection = DWConv(channels, self.out_channels, kernel=3, padding=1)
        else:
            self.skip_connection = conv_nd(dims, channels, self.out_channels, 1)

        # self.cross_att_emb = CrossAttention(self.out_channels)
        # self.cross_att_detail = CrossAttention(self.out_channels)
    ...
    def forward(self, x, emb=None, detail_cond=None):
        ...
        if self.use_scale_shift_norm:
            ...
            if emb is not None:
                ...
                h = h * (1 + scale) + shift
                # h = h + self.cross_att_emb(h, emb_out)

            if detail_cond is not None:
                scale, shift = th.chunk(detail_cond_out, 2, dim=1)
                h = h * (1 + scale) + shift
                # h = h + self.cross_att_detail(h, detail_cond_out)
...
class CrossAttention(nn.Module):
    def __init__(self, in_channels):
        ...
        self.in_conv = nn.Conv2d(in_channels * 2, in_channels, 1)

        self.theta = nn.Conv2d(in_channels, in_channels, 1)
        self.phi = nn.Conv2d(in_channels, in_channels, 1)
        self.gama = nn.Conv2d(in_channels, in_channels, 1)
    ...
    def forward(self, x, y):
        ...
        theta = self.theta(x).view(b, c, -1)  # [B, C, H*W]
        phi = self.phi(y).view(b, c, -1)  # [B, C, H*W]

        attn = th.bmm(theta.permute(0, 2, 1), phi)  # [B, H*W, H*W]
        attn = self.softmax(attn)

        gama = self.gama(y).view(b, c, -1)  # [B, C, H*W]
        attn_g = th.bmm(gama, attn.permute(0, 2, 1))  # [B, C, H*W]
...
class UNetModel(nn.Module):
    ...
    def forward(self, x, timesteps, physic_cond, detail_cond):
        ...
        hs = []
        emb = self.time_embed(timestep_embedding(timesteps, self.model_channels))  # [B, 512]

        if physic_cond is not None:
            x = th.cat([x, physic_cond], dim=1)  # [B, 3+9, 256, 256]

        h = x
        for module in self.input_blocks:  # 18 layers
            h = module(h, emb, detail_cond)
            hs.append(h)
        h = self.middle_block(h, emb, detail_cond)  # 3 layers  [B, 512, 8, 8] [B, 512] [B, 64]
...
class DiffusionRig(nn.Module):
    ...
    def forward(self, x, t, physic_cond, detail_cond=None, x_start=None):  # ####### main stream #######
